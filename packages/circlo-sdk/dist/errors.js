...
export class NotConfiguredError extends CircloSdkError {
    operation;
    missing;
    constructor(operation, missing) {
        const detail = missing === "walletClient"
...
export class EventNotFoundError extends CircloSdkError {
    txHash;
    eventName;
    constructor(eventName, txHash) {
        super(`tx ${txHash} confirmed but no ${eventName} event was found in receipt — ` +
...
export class TxRevertedError extends CircloSdkError {
    txHash;
    operation;
    constructor(operation, txHash) {
        super(`${operation} reverted on-chain. Inspect tx ${txHash} on the explorer ` +
